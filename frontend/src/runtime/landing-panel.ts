...
export class LandingPanel {
  ...
  constructor(parent: HTMLElement, submit: (settings: LandingSettings) => void) {
    ...
    panel.innerHTML = `<header><strong>降落辅助 <small>参考</small></strong><label class="landing-auto"><input type="checkbox" data-part="automatic">自动切换</label><button type="button" data-part="toggle" aria-pressed="false">开启</button></header>
      ...
      <div class="landing-airframe"><span data-part="gearLimit"></span><strong data-part="gearAdvice"></strong><span data-part="arrestor"></span><small>是否放钩、挂索及当前跑道的阻拦索状态需在游戏内确认。</small><span data-part="touchdown"></span></div>
      ...
      </div><button type="button" data-part="apply">应用参考参数</button><p>高程须与 8111 高度使用相同基准。参考线在入口上方 15 m；角度和 IAS 需按机型、载荷自行选择。地形不含跑道设施与障碍物，越过入口后不提供拉平或接地判定。</p></details>
