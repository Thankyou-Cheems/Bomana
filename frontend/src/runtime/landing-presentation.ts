...
export function landingPresentation(landing: LandingSnapshot | null | undefined) {
  ...
  const lateral = returning ? g.airportTrackErrorDeg === null ? "等待地面航迹" : Math.abs(g.airportTrackErrorDeg) < 2 ? "正飞向机场"
    : `向${g.airportTrackErrorDeg > 0 ? "左" : "右"}修正 ${Math.abs(g.airportTrackErrorDeg).toFixed(1)}°`
    : g ? Math.abs(g.crossTrackM) < 20 ? "轴线附近" : g.stage === "final"
  ...
  return { stage, lateral, vertical, speed, configuration, distance, course, verticalSpeed, elevation, compact, gearLimit, gearAdvice, arrestor, touchdown,
