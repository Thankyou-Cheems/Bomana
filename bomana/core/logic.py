...
from bomana.core import lifecycle, navigation, release_state, timing_store
...
if ENABLE_CCRP:
    ...
    from bomana.core.terrain_elevation import TerrainElevationService, TerrainReliefOverlay
    ...
    from bomana.core import ccrp_scheduler, weapon_scheduler
else:
    ccrp_scheduler = None
    weapon_scheduler = None
    calculate_bomb_trajectory = None
    calculate_release_timing_from_range = None
    TerrainElevationService = None
    TerrainReliefOverlay = None
    WeaponCatalogError = RuntimeError
    get_weapon_catalog = None
    WeaponSolver = None
    uses_existing_ccrp = None
...
class GameLogic:
    ...
    def __init__(self, *, clock: WallClock | None = None, http: HttpJson | None = None):
        ...
        self._manual_interest_point: InterestPoint | None = None
        self._manual_interest_point_context: str | None = None
    ...
    def set_bombing_target_mode(self, mode: object) -> bool:
        ...
        with self._lock:
            ...
            if normalized == "poi":
                self._clear_zone_target_selection_locked()
    ...
    def set_manual_interest_point(self, x: object, y: object) -> bool:
        """Set one App-owned battle-scoped POI without mutating official map data."""
        ...
        with self._lock:
            mp = self.state.last_map
            if (
                self.state.phase == Phase.HANGAR
                or mp is None
                or not mp.ok
                or not mp.player_aircraft_present
                or mp.player_pos is None
            ):
                return False
            ...
            # Official 8111 POIs are authoritative. A Web request made while
            # one is already present is a successful no-op so the browser can
            # keep the native point and show a non-error precedence hint.
            if any(point.id != self.MANUAL_POI_ID for point in mp.interest_points):
                self._manual_interest_point = None
                self._manual_interest_point_context = None
                return True

            self._manual_interest_point = InterestPoint(
                id=self.MANUAL_POI_ID,
                index=1,
                x=round(normalized_x, 6),
                y=round(normalized_y, 6),
                name="自定 POI",
            )
            self._manual_interest_point_context = timing_store.build_map_signature(
                self.state.map_info
            )
            # Publish the App-owned point immediately. The next successful 8111
            # map sample will still pass through the normal merge path, where a
            # native POI remains authoritative.
            self.state.last_map = replace(
                mp,
                interest_points=[self._manual_interest_point],
            )
        return True

    def _clear_zone_target_selection_locked(self) -> None:
        """Clear stale automatic zone ownership when explicit POI mode takes over."""
        ...
        nav.zones = [
            replace(zone, is_target=False) if zone.is_target else zone for zone in nav.zones
        ]

    def _merge_manual_interest_point_locked(
        self,
        mp: MapObjData,
        raw_mp: MapObjData,
    ) -> MapObjData:
        """Merge the App fallback only after preserving native-POI precedence."""

        def without_manual(value: MapObjData) -> MapObjData:
            points = [point for point in value.interest_points if point.id != self.MANUAL_POI_ID]
            if len(points) == len(value.interest_points):
                return value
            return replace(value, interest_points=points)

        if self.state.phase == Phase.HANGAR:
            self._manual_interest_point = None
            self._manual_interest_point_context = None
            return without_manual(mp)

        native_points = (
            [point for point in raw_mp.interest_points if point.id != self.MANUAL_POI_ID]
            if raw_mp.ok
            else []
        )
        if native_points:
            self._manual_interest_point = None
            self._manual_interest_point_context = None
            # A raw native POI takes effect immediately even if the remaining
            # map fields are temporarily using the previous-frame grace value.
            return replace(without_manual(mp), interest_points=native_points)

        manual_point = self._manual_interest_point
        if manual_point is None:
            return without_manual(mp)

        current_context = timing_store.build_map_signature(self.state.map_info)
        if (
            self._manual_interest_point_context
            and current_context
            and current_context != self._manual_interest_point_context
        ):
            self._manual_interest_point = None
            self._manual_interest_point_context = None
            return without_manual(mp)
        if not mp.ok:
            return mp
        return replace(mp, interest_points=[manual_point])
    ...
    def current_terrain_relief_overlay(self) -> TerrainReliefOverlay | None:
        """Return the active App-generated relief without exposing terrain-pack bytes."""

        service = self.terrain_elevation
        return service.current_relief_overlay() if service is not None else None

    def current_terrain_map_id(self) -> str:
        service = self.terrain_elevation
        match = service.current_match if service is not None else None
        return match.map_id if match is not None else ""
    ...
    def tick(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                mp = self._merge_manual_interest_point_locked(mp, raw_mp)
                s.last_tel = tel
                s.last_map = mp
    ...
    def _update_zone_navigation_locked(
        ...
        *,
        ...
        zone_navigation_targeting = zone_targeting_enabled and bombing_target_mode == "zone"
        ...
        if zone_navigation_targeting and is_airborne and zones_with_nav:
            # 创建ID到Zone的映射，方便查找
        ...
        if (
            ...
            poi_candidates: list[tuple[int, float, float, InterestPoint, float]] = []
            for point in mp.interest_points:
                ...
                poi_candidates.append(
                    (
                        0 if point.id == self.MANUAL_POI_ID else 1,
                        abs(relative),
                        distance,
                        point,
                        relative,
            ...
            if poi_candidates:
                _, _, distance, point, relative = min(
                    poi_candidates,
