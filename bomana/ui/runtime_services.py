...
if ENABLE_WEB_DASHBOARD:
    ...
    class _NullDashboardStore:
        ...
        def publish_terrain_overlay(self, *_args: Any, **_kwargs: Any) -> None:
            return None

        def clear_terrain_overlay(self) -> None:
            return None

        def read_terrain_overlay(self) -> None:
            return None
...
class AppRuntimeServices:
    ...
    def _on_map_image(self, image_bytes: bytes) -> None:
        """Publish App-rendered offline relief after active-map identification."""

        game = getattr(self.app, "game", None)
        identify = getattr(game, "update_terrain_map_image", None)
        relief = getattr(game, "current_terrain_relief_overlay", None)
        active_map_id = getattr(game, "current_terrain_map_id", None)
        if not callable(identify) or not identify(image_bytes) or not callable(relief):
            self.dashboard_store.clear_terrain_overlay()
            return
        current = self.dashboard_store.read_terrain_overlay()
        resolved_map_id = str(active_map_id() or "") if callable(active_map_id) else ""
        if current is not None and resolved_map_id and current.map_id != resolved_map_id:
            self.dashboard_store.clear_terrain_overlay()
        overlay = relief()
        if overlay is None:
            self.dashboard_store.clear_terrain_overlay()
            return
        self.dashboard_store.publish_terrain_overlay(
            overlay.body,
            map_id=overlay.map_id,
            minimum_altitude_m=overlay.minimum_altitude_m,
            maximum_altitude_m=overlay.maximum_altitude_m,
            width=overlay.width,
