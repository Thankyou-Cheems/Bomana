...
from bomana.ui.theme import Instrument, Theme
...
class HeadingTape(tk.Canvas):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            bg=Instrument.BG,
            highlightthickness=1,
            highlightbackground=Instrument.SEPARATOR,
            highlightcolor=Instrument.SEPARATOR,
            **kwargs,
        )
        ...
        self._target_colors = {
            "zone": Instrument.RED,
            "poi": Instrument.RED,
            "traceback": Instrument.YELLOW,
            "friendly": Instrument.BLUE,
            "enemy": Instrument.ORANGE,
            "hostile_aircraft": Instrument.RED,
            "destroyed": Instrument.TEXT_MUTED,
        }
    ...
    def _layout_metrics(self) -> dict[str, Any]:
        ...
        guidance_band_height = (
            guidance_pad + guidance_linespace + row_gap + guidance_track_height + guidance_pad
        )
        ...
        guidance_track_y = (
            guidance_text_y + guidance_linespace + row_gap + guidance_track_height / 2
        )
    ...
    def _guidance_text(
        ...
        *,
        ...
        distance_text = (
            format_distance_ete(distance, primary.get("ete_str")) if distance > 0 else ""
        )
        suffix = f" · {distance_text}" if distance_text else ""

        if abs_rel < 0.05:
            return f"已对准{suffix}", Instrument.GREEN
    ...
    def _draw_alignment_cue(self, layout: dict[str, Any] | None = None) -> None:
        ...
        with contextlib.suppress(tk.TclError):
            ...
            if primary is None:
                ...
                color = Instrument.YELLOW if self._last_mode_notice else Instrument.TEXT_MUTED
                self.create_text(
                    center_x,
    ...
    def _draw_target_marker(
        ...
        base_color = self._target_colors.get(t_type, Instrument.TEXT)

        if not is_target:
            # 非目标：使用中等亮度颜色（比之前更亮）
            color_map = {
                Instrument.RED: "#CC6666",  # 中红（更亮）
                Instrument.BLUE: "#6688BB",  # 中蓝（更亮）
                Instrument.ORANGE: "#CC9966",  # 中橙（更亮）
            }
            color = color_map.get(base_color, Instrument.TEXT_DIM)
        else:
            color = base_color
        layout = layout or self._layout_metrics()
        ...
        if t_type == "zone":
            ...
            if is_primary:
                ...
                self.create_text(
                    ...
                    dist_y,
                    text=label_text,
                    fill=marker_color if is_target else Instrument.TEXT_MUTED,
                    font=text_font,
                    anchor="s",
                    tags=(label_tag,),
                )
            ...
            self.create_text(
                ...
                dist_y,
                text=label_text,
                fill=self._target_colors[t_type] if is_target else Instrument.TEXT_MUTED,
                font=text_font,
                anchor="s",
    ...
    def _draw_overflow_indicator(
        ...
        *,
        ...
        Args:
            ...
            distance: 目标距离（公里）
        """
        color = self._target_colors.get(t_type, Instrument.TEXT_DIM)
        layout = layout or self._layout_metrics()
        icon_scale = float(layout["marker_scale"])
    ...
    def _draw_primary_overflow(self, diff: float, *, layout: dict[str, Any] | None = None):
        ...
        if diff < 0:
            ...
            self.create_polygon(arrow_points, fill=Instrument.RED, outline=Instrument.BG)
            self.create_text(
                40,
                y,
                text=f"◀ {abs(int(diff))}°",
