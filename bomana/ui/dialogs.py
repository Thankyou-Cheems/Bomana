...
if ENABLE_CCRP:
    ...
    def get_weapon_catalog() -> Any:
        raise RuntimeError("Strike Prediction implementation is not installed")
...
class _ScalableDialogMixin:
    ...
    def _init_dynamic_scaling(self):
        ...
        def collect(widget):
            for child in widget.winfo_children():
                collect(child)
            if "font" in widget.keys():  # noqa: SIM118 - Tk widgets are not mappings.
                try:
...
class SettingsDialog(tk.Toplevel, _ScalableDialogMixin, SettingsRuntimeMixin):
    ...
    def _build_ui(self):
        ...
        for index, tab in enumerate(self.tabs):
            ...
            self.tab_buttons_frame.grid_columnconfigure(index, weight=1, uniform="tabs")
            btn.grid(row=0, column=index, sticky="ew", padx=1, pady=1)
    ...
    def _reset_defaults(self):
        ...
        if messagebox.askyesno("确认", "确定要重置所有设置为默认值吗？", parent=self):
            ...
            self.theme_var.set(Theme.DEFAULT)
