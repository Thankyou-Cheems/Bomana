...
def generate_cdi_indicator(relative_angle: float, distance_km: float) -> tuple[str, str]:
    ...
    if is_overflow_left:
        ...
        indicator[1] = (
            ZoneConfig.CDI_OVERFLOW_LEFT[1]
            if len(ZoneConfig.CDI_OVERFLOW_LEFT) > 1
            else ZoneConfig.CDI_TRACK
        )
    else:
    ...
    if is_overflow_right:
        indicator[-1] = (
            ZoneConfig.CDI_OVERFLOW_RIGHT[-1]
            if len(ZoneConfig.CDI_OVERFLOW_RIGHT) > 1
            else ZoneConfig.CDI_OVERFLOW_RIGHT[0]
        )
