...
def test_production_has_no_second_outbound_game_data_client() -> None:
    ...
    assert socket_imports == [Path("bomana/web/server.py")]
    web_server_source = (ROOT / socket_imports[0]).read_text(encoding="utf-8")
    for outbound_call in (".connect(", ".connect_ex(", ".sendto(", ".sendall("):
        assert outbound_call not in web_server_source
