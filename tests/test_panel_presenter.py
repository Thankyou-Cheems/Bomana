...
def test_fuel_display_model_formats_return_warning() -> None:
    ...
    assert model.main_text == "420kg (18%)"
    assert model.main_fg == Theme.YELLOW
    assert model.time.text == "12:34"
    assert model.return_status.icon == "warning"
    assert model.return_status.text == "注意"
    assert model.detail_text == "油耗 38kg/min"
    assert model.altitude_text == "高度 1234m"
    assert model.return_detail_text == "返航 需~250kg (50%)"
...
def test_bombing_display_model_ready_state(monkeypatch) -> None:
    ...
    assert model.trajectory_text == "弹道: 1.42km"
    assert model.flight_text == "飞行: 3.6s"
    assert model.release.icon == "bomb"
    assert model.release.text == "投弹"
    assert model.release.fg == Theme.GREEN
    assert model.release_detail_text == "时间 0.42s，距离 95m"
...
def test_bombing_display_model_explains_mach_limit(monkeypatch) -> None:
    ...
    assert model.trajectory_text == "弹道: 超限"
    assert model.flight_text == "飞行: 不计算"
