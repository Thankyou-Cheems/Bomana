...
def test_poi_mode_makes_interest_point_primary_and_suppresses_zone_takeover() -> None:
    poi = SimpleNamespace(
        id="poi-web-manual",
        name="自定 POI",
        distance_km=4.25,
    ...
    poi_target = next(target for target in model.targets if target["type"] == "poi")
    zone_target = next(target for target in model.targets if target["type"] == "zone")
    assert poi_target["is_primary"] is True
    assert poi_target["is_target"] is True
    assert zone_target["is_primary"] is False
    assert zone_target["is_target"] is False
    assert model.primary_zone is None
    assert model.primary_target is poi
    assert model.primary_target_info is not None
    assert model.primary_target_info["type"] == "poi"
    assert [info["type"] for info in model.active_targets_info] == ["poi"]


def test_poi_mode_without_a_point_does_not_fall_back_to_a_zone() -> None:
    zone = SimpleNamespace(
        id="zone-1",
        relative=-3.0,
        distance_km=7.0,
