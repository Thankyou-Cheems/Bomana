...
from bomana.core import diagnostics, lifecycle, navigation, timing_store
from bomana.core.state import Airfield, GameState, MapInfo, MapObjData, TelemetryData, Zone
...
def test_map_signature_ignores_mutable_zones_but_tracks_map_metadata() -> None:
    map_info = _valid_map_info()
    changed_map_info = _valid_map_info()
    changed_map_info.map_max = [2500.0, 1000.0]

    first = timing_store.build_map_signature(map_info)
    second = timing_store.build_map_signature(map_info)

    assert first is not None
    assert first == second
    assert timing_store.build_map_signature(changed_map_info) != first
    assert timing_store.build_map_signature(None) is None
