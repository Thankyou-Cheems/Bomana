...
REM ============================================================================
REM 用法:
REM   tools\scripts\build_portable.bat [Standard|Lite] [all|app|launcher] [version]
REM 示例:
REM   tools\scripts\build_portable.bat Standard
...
if /I not "%VARIANT%"=="Standard" if /I not "%VARIANT%"=="Lite" (
    echo [错误] 版本类型无效: %VARIANT%
    echo 用法: tools\scripts\build_portable.bat Standard^|Lite [all^|app^|launcher] [version]
    set "EXIT_CODE=1"
    goto :cleanup
)
