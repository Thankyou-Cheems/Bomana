...
def refresh_download_catalog(cdn_base: str, catalog_path: Path) -> dict:
    ...
    print(
        ...
        catalog["channels"].get("Standard", {}).get("app_version"),
        flush=True,
    )
    return catalog
...
def stage_site(docs: Path, stage: Path) -> None:
    ...
    required = [
        ...
        stage / "calculator.html",
        stage / "calculator.js",
        stage / "ec-calculator.json",
        stage / "assets" / "shots" / "nav-hud.png",
        stage / "assets" / "shots" / "nav-precision.png",
        stage / "assets" / "shots" / "desktop-main.png",
        stage / "assets" / "shots" / "launcher.png",
    ]
...
def deploy_stage(host: str, remote_dir: str, stage: Path) -> None:
    ...
    script = "\n".join(
        [
            ...
            'test -f "$REMOTE_DIR/assets/shots/desktop-main.png"',
            'test -f "$REMOTE_DIR/assets/shots/launcher.png"',
            "",
        ]
    )
...
def verify_public(public_base: str, cdn_base: str) -> None:
    ...
    checks = [
        ...
        f"{base}/calculator.html",
        f"{base}/calculator.js",
        f"{base}/ec-calculator.json",
        f"{base}/assets/shots/nav-hud.png",
        f"{base}/assets/shots/nav-precision.png",
        f"{base}/assets/shots/desktop-main.png",
        f"{base}/assets/shots/launcher.png",
