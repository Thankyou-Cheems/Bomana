...
- 打包检查使用真实 Launcher `3.1.0` 与
  `Bomana_app_<Variant>_v8.6.2.zip`，不要用源码目录代替。
...
分别从 Enhanced、Standard、Lite 的真实 App 8.6.2 包启动；断开外网后打开
页面并查看浏览器 Network 面板。
...
Boat、Destroyer 等当前官方 `icon` 使用 8111 `/icons.ttf` 与官方 glyph 映射，
地图标记和图例样本必须一致且不得退回自绘敌军矢量；蓝方/己方
单位与地图要素不被误判。逐项点击地图角落的图例后，只隐藏/恢复对应类别，
刷新配对状态不应把过滤变成 App 配置或 Web 写命令。单位从下一原始样本消失
或采样失败时立即移除，独立导航栏与 CCRP 提示栏不出现这些标记；
...
- Launcher 3.3.0 + App 8.6.2 正常交接，App 在运行时初始化前拿到严格
  `BOMANA_LAUNCHER_VERSION`；
- 缺失、畸形、`2.9.9` 或仍在使用 `3.2.2` 的 Launcher 身份被 App
  8.6.2 拒绝，即使设置
